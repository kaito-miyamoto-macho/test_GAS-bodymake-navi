function transferSessionAfterData() {
    ...
    // 「回答者ID」の列を取得
    const responderIdIndex = outputHeaders.indexOf(ELME_HIDDEN_COL.ANSWER_ID);
    ...
    // **ここで担当コーチに通知を送信**
    sendCoachNotificationSessionAfter(responderId);
}
 ...
 * 担当コーチにLINEで通知を送る
 */
function sendCoachNotificationSessionAfter(responderId) {
  const pjtSS = SpreadsheetApp.openById(SPREAD_SHEET_IDS.PJTManagement);
  const customerSheet = pjtSS.getSheetByName(SHEET_NAMES_MANAGEMENT.CLIENT_LIST);
